...
/**
 * {@link DatasetMetricFacet}s are used to track numeric metrics for the output dataset of a given job run. Depending on
 * the context, a metric may apply to the entire dataset version or only the partition written by the job run.
 * A single {@link DatasetMetricFacet} may contain multiple metrics for a given dataset version. Each metric will
 * reference a single {@link Metric}, possibly containing multiple values, such as {@link Metric#count}
 * or {@link Metric#sum}, or a {@link Quantile} value.
 */
@Data
public class DatasetMetricFacet extends LineageEvent.BaseFacet {
  ...
  /**
   * Encapsulates a single fact that may be measured in a number of ways- currently, {@link #count}, {@link #sum}, and
   * {@link #quantiles} are supported. One or more of these fields can be populated for a single metric.
   */
  @Data
  ...
  /**
   * Utility method constructing a single {@link Metric} with a count value. The constructed metric will be added
  ...
  /**
   * Utility method constructing a single {@link Metric} with a sum value. The constructed metric will be added
   * to the current facet.
   * @param name The name of the metric to create
  ...
  /**
   * Utility method constructing a single {@link Metric} with a specified metric value. The constructed metric will
   * be added to the current facet.
   * @param name The name of the metric to create
   * @param metric The
   */
  ...
  /**
   * Represents a quantile value- the quantile group (e.g., .20 or .10) and the value for that quantile group.
   */
